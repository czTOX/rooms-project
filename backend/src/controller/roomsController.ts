...
roomsRouter.get('/', async (req, res) => {
  ...
  if (req.query.location !== undefined) {
    args.location =
      req.query.location.toString().length > 0
        ? req.query.location.toString()
        : undefined;
  }
  if (req.query.sort) {
    args.sort =
      req.query.sort.toString().length > 0
        ? req.query.sort.toString()
        : undefined;
  }
  if (req.query.startDate) {
    args.startDate =
      req.query.startDate.toString().length > 0
        ? req.query.startDate.toString()
        : undefined;
  }
  if (req.query.endDate) {
    args.endDate =
      req.query.endDate.toString().length > 0
        ? req.query.endDate.toString()
        : undefined;
  }
  if (req.query.minPrice) {
    args.minPrice =
      req.query.minPrice.toString().length > 0
        ? req.query.minPrice.toString()
        : undefined;
  }
  if (req.query.maxPrice) {
    args.maxPrice =
      req.query.maxPrice.toString().length > 0
        ? req.query.maxPrice.toString()
        : undefined;
  }
  if (req.query.search) {
    args.search =
      req.query.search.toString().length > 0
  ...
  if (args.endDate != undefined && args.startDate !== undefined) {
    if (args.endDate <= args.startDate) {
      ...
      Date.parse(args.endDate) < new Date().setHours(0, 0, 0, 0) ||
      Date.parse(args.startDate) < new Date().setHours(0, 0, 0, 0)
    ) {
...
roomsRouter.post(
  '/',
  upload.array('images'),
  validation({ body: RoomPostSchema }),
  async (req, res) => {
    if (!req.session.user) {
...
roomsRouter.post(
  '/:roomId/offers',
  validation({ body: OfferPostSchema }),
  async (req: Request, res) => {
    ...
    let data = req.body;
    data.roomId = roomId;
    const offer = await offersRepository.createSingle({ ...data });
    if (offer.isErr) {
    ...
    return resultOk(
      offer.value,
      res,
      `Created offer for room. Offer id is: ${offer.value?.id}`
    );
  }
);
