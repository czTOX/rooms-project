...
locationsRouter.get('/', async (req, res) => {
  ...
  return resultOk(
    location.value,
    res,
    `Listed ${location.value.length} locations`
  );
});

locationsRouter.post(
  '/',
  validation({ body: LocationCreateSchema }),
  async (req, res) => {
    const location = await locationsRepository.createSingle({ ...req.body });
    if (location.isErr) {
      return resultError(500, res, location.error.message);
    }
    return resultOk(
      location.value,
      res,
      `Created room with id: ${location.value?.id}`
    );
  }
);
